...
local function build_frontmatter(meta)
  ...
  if insert_title_page then
    ...
    local doctoral_defense_date = meta_str(meta, 'doctoral_defense_date')
    ...
    if degree_type ~= '' or cn_title ~= '' then
      ...
      inject_blank_lines(blocks, 1)
      if degree_type ~= '' then
        blocks:insert(styled_para(degree_type, 'TitlePageDegree'))
      end
      inject_blank_lines(blocks, 1)
      if cn_title ~= '' then
        blocks:insert(styled_para(cn_title, 'Title'))
...
function Pandoc(doc)
  ...
  local function flush_ack_signature()
    ...
    local sign_date = meta_str(doc.meta, 'ack_signature_date')
    if sign_date == '' then
      sign_date = meta_str(doc.meta, 'submit_date_cn')
    end
    ...
    push_signature_line('作者：' .. sign_name)
    push_signature_line('写作日期：' .. sign_date)
    ack_signature_inserted = true
  end
    ...
    elseif block.t == 'Para' then
      ...
      if mode == 'cn_abstract' then
        if starts_with(text, '关键词') then
          inject_blank_lines(out, 1)
          push(clone_para_with_style(block, 'Keywords Line CN'))
        else
