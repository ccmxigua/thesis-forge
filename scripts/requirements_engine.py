...
def merge_llm_primary(source: Path, rule_spec: dict[str, Any], clauses: list[dict[str, Any]],
    ...
    for item_index, item in enumerate(requirements):
        ...
        reasons = []
        if not role or not isinstance(props, dict):
            reasons.append("missing_role_or_properties")
        else:
            reasons.extend(
                error.rsplit(": ", 1)[-1]
                for error in requirement_payload_errors(item, item_index)
            )
