...
% ===== 合成测试元数据 =====
% 以下个人、组织、编号和日期均为虚构数据，仅用于端到端格式测试。
% 学位类别必须由源文档显式声明；这里用 academic 覆盖完整学术型硕士样例，
% 不依赖系统根据“工学硕士”或专业名称自行猜测 academic/professional。
\title{基于机器学习的城市交通流预测方法研究}
\englishtitle{Research on Urban Traffic Flow Prediction Methods Based on Machine Learning}
\subtitle{面向多模板学位论文转换的示例研究}
\englishsubtitle{An Example Study for Multi-Template Thesis Conversion}
\author{测试学生甲}
\authoren{Test Student A}
\advisor{测试导师乙 教授}
\advisoren{Prof. Test Advisor B}
\coadvisor{测试合作导师丙 副教授}
\coadvisoren{Assoc. Prof. Test Co-advisor C}
\school{智能交通与数据科学学院}
\schoolen{School of Intelligent Transportation and Data Science}
\major{交通信息工程及控制}
\majoren{Traffic Information Engineering and Control}
\firstdiscipline{交通运输工程}
\seconddiscipline{交通信息工程及控制}
\researchdirection{交通大数据与智能计算}
\researchdirectionen{Transportation Big Data and Intelligent Computing}
\studentid{TEST20260001}
\classificationnumber{U491.1}
\udc{656.13}
\confidentialitylevel{公开}
\confidentialitylevelen{Public}
\submitdate{2026年6月}
\submitdateen{June 2026}
\defensedate{2026年5月20日}
\defensedateen{May 20, 2026}
\degreeconferraldate{2026年6月30日}
\degreeconferraldateen{June 30, 2026}
\degreecategory{academic}
\degreedisplay{学术学位工学硕士学位论文}
\degreedisplayen{Academic Master of Engineering Thesis}

\begin{document}

% ===== 中文摘要 =====
\begin{abstract}
随着城市化进程的加速，城市交通拥堵问题日益严重，精准的交通流预测对智能交通系统建设具有重要意义。本文系统梳理了交通流预测领域的研究现状，分析了传统统计模型与深度学习模型的优缺点。在此基础上，提出了一种融合注意力机制的时空图卷积网络模型。该模型通过图卷积网络捕捉路网拓扑结构中的空间依赖关系，利用门控循环单元提取时间序列特征，并通过多头注意力机制增强长距离依赖建模能力。实验结果表明，所提模型在PeMSD4和PeMSD8数据集上的预测精度均优于基线模型，平均绝对误差降低幅度达到百分之八以上。此外，本文还对模型的可解释性进行了分析，探讨了不同类型的注意力头在时空建模中的实际作用。全文包含三章编号正文，系统地阐述了从问题提出到模型构建及方法说明的研究过程；实验环境与补充结果另列于附录。
...
\section{论文的结构安排}
本文编号正文共分为三章。第一章介绍研究背景和意义，综述国内外研究现状，明确本文的研究内容和创新点。第二章介绍相关的理论基础，包括图卷积网络、循环神经网络和注意力机制。第三章详细阐述本文提出的模型结构与实现方法。此外，本文还包含参考文献、附录、致谢和攻读硕士学位期间的学术成果等部分。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{assets/gcn-aggregation.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{assets/gru-structure.png}
\caption{门控循环单元的内部结构}
\label{fig:gru}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{assets/multihead-attention.png}
...
% ===== 后记 =====
\acknowledgments{
时光荏苒，硕士求学生涯即将画上句号。回首过去的几年，感慨万千。

首先，衷心感谢我的导师张教授。在硕士学习期间，张老师不仅在学术研究上给予我悉心指导，还在生活中给予我无私关怀。张老师严谨的治学态度、开阔的学术视野和谦逊的人格魅力，将永远是我学习的榜样。
...
硕士生涯的结束不是终点，而是新征程的起点。我将带着在这里收获的知识和友谊，继续探索未知的世界。
}

% ===== 在学期间研究成果（仅用于端到端格式测试） =====
% 以下条目均为虚构测试数据，不代表任何真实作者的学术成果。
\chapter{在学期间发表的学术论文与研究成果}

本节内容仅用于验证论文转换管线对学术成果章节的识别、装配和格式处理能力，所有名称、题目和出版信息均为虚构测试数据。

\noindent （1）测试作者甲，测试作者乙．面向论文格式转换的结构化文档处理方法[J]．示例学术期刊，2025，1(1)：1--10．

\noindent （2）测试作者甲．多模板学位论文自动排版系统的设计与实现[C]．示例学术会议，2025：20--28．
